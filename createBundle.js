...
twilioClient.addresses
  ...
  .then((body) => {
    declarationOfBeneficialOwnershipSid = body ? JSON.parse(body).sid : '';
    console.log(
      `>>> DeclarationOfBeneficialOwnershipSid:${declarationOfBeneficialOwnershipSid} created.`,
    );
    // console.dir(JSON.parse(body));
  ...
  .then((body) => {
    console.log(`>>> Power of Attorney Assign completed.`);
    // 実質的支配者のアサイン
    const options = {
      url: `https://numbers.twilio.com/v2/RegulatoryCompliance/Bundles/${bundleSid}/ItemAssignments`,
      method: 'POST',
      ...
      form: {
        ObjectSid: declarationOfBeneficialOwnershipSid, // ドキュメント
      },
    };
    return rq(options);
  })
  .then((body) => {
    if (body)
      console.log(
        `>>> Completed Japan Regulatory Bundle Application Assign completed.`,
      );
    // 運転免許証のアサイン
    const options = {
      url: `https://numbers.twilio.com/v2/RegulatoryCompliance/Bundles/${bundleSid}/ItemAssignments`,
  ...
  .then((body) => {
    console.log(`>>> Submitted. ${bundleSid}`);
  })
  .catch((err) => {
    console.error(err);
  });
